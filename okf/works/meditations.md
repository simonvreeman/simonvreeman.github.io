---
type: Work
title: Meditations
description: "The private Stoic notebook of Marcus Aurelius (Greek: Ta eis heauton, 'to himself'), here in Gregory Hays's Modern Library translation."
resource: https://vreeman.com/meditations/
tags: [stoicism, work]
timestamp: 2026-06-19T12:42:41.992Z
sameAs: https://www.wikidata.org/wiki/Q1152283
---
# Meditations

The private Stoic notebook of Marcus Aurelius (Greek: Ta eis heauton, 'to himself'), here in Gregory Hays's Modern Library translation.

## Related

- Written by [Marcus Aurelius](../persons/marcus-aurelius.md).
- Translated by [Gregory Hays](../persons/gregory-hays.md).
- An instance of [Stoicism](../concepts/stoicism.md).

## Examples

> Meditations is a series of personal writings by Marcus Aurelius, Roman Emperor from 161 to 180 AD, recording his private notes to himself and ideas on Stoic philosophy. [1]

## Citations

[1] [📘 Meditations by Marcus Aurelius. Translation by Gregory Hays.](https://vreeman.com/meditations/)
