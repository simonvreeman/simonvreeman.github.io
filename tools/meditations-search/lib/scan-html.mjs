...
// and throws on anything it does not recognise, rather than silently indexing something else.
...
const STRIP_ELEMENTS = /<sup\b[^>]*>[\s\S]*?<\/sup>|<a\b[^>]*\bclass="[^"]*\breturn\b[^"]*"[^>]*>[\s\S]*?<\/a>/gi;
...
// The block elements that appear as direct children of a Book section, per tools/.../markup.test.mjs.
const CHILD_TAGS = ['h2', 'h3', 'p', 'ul', 'ol', 'blockquote'];

// Yields [outerHtml, tagName] for each top-level element of `html` built from `tags`, tracking depth so
...
function* topLevelElements(html, tags) {
  ...
  for (const m of html.matchAll(token)) {
    ...
    if (depth === 0) {
      ...
      depth += closing ? -1 : 1;
      if (depth === 0) yield [html.slice(start, m.index + m[0].length), name];
    }
  }
...
export function scanBooks(html) {
  ...
  for (const open of main.matchAll(BOOK_SECTION)) {
    for (const [child, tag] of topLevelElements(sectionBody(main, open), CHILD_TAGS)) {
      if (tag === 'h2') continue;
